...
import PlayerHologram from '../../Gameplay/PlayerHologram/PlayerHologram';
import { settingsState } from '../../../atoms/settingsState';
import LandmarkDebugCanvas from '../../Gameplay/LandmarkDebugCanvas/LandmarkDebugCanvas';
...
const CalibrationScene = () => {
  ...
  function checkReadinessForStep1(results: any) {
    ...
    return (
      between(angleRightLink1, 1.5 * Math.PI * (1 - range), 1.5 * Math.PI * (1 + range)) &&
      between(angleRightLink2, 1.5 * Math.PI * (1 - range), 1.5 * Math.PI * (1 + range)) &&
      between(angleLeftLink1, 0.5 * Math.PI * (1 - range), 0.5 * Math.PI * (1 + range)) &&
      between(angleLeftLink2, 0.5 * Math.PI * (1 - range), 0.5 * Math.PI * (1 + range))
    );
  }
  ...
  function checkReadinessForStep2(results: any) {
    ...
    const range = 0.15;
    return (
      (angleRightLink1 > 2 * Math.PI * (1 - range) || angleRightLink1 < 2 * Math.PI * range) &&
      (angleRightLink2 > 2 * Math.PI * (1 - range) || angleRightLink2 < 2 * Math.PI * range) &&
      (angleLeftLink1 > 2 * Math.PI * (1 - range) || angleLeftLink1 < 2 * Math.PI * range) &&
      (angleLeftLink2 > 2 * Math.PI * (1 - range) || angleLeftLink2 < 2 * Math.PI * range)
    );
  }
  ...
  return (
    <div className="CalibrationScene" data-testid="Gameplay" ref={gameplayWrapper}>
      <video
