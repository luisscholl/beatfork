...
const checkAuthenticated = function (error) {
  return async (req, res, next) => {
    if (!req.app.locals.authenticated) {
      let notAuthenticated = new Error()
  ...
  return async (req, res, next) => {
    ...
    for (version of req.body.versions) {
      if (versionIds.hasOwnProperty(version.id)) {
        let duplicateVersionId = new Error()
...
const checkAuthorized = function (error, message) {
  return async (req, res, next) => {
    ...
    const oldLevel = await req.app.locals.db.collection('levels').findOne(query)
    if (!oldLevel) {
      let levelNotFound = new Error()
      levelNotFound.status = 404
      levelNotFound.errors = 'Level not found'
    ...
    if (oldLevel.author.id !== req.app.locals.userId) {
      let userNotAuthorized = new Error()
...
router.post('/', checkAuthenticated('Levels can only be created by authorized users'),
  checkNoDuplicateVersionIds(), setArtists(), setAuthor(),
  async (req, res, next) => {
    const result = await req.app.locals.db.collection('levels').insertOne(req.body)
...
router.get('/', async (req, res, next) => {
  ...
  aggregationPipeline.push({
    $match: {
      ...
      ...(req.app.locals.authenticated && { $or: [{ published: true }, { 'author.id': req.app.locals.userId }] }),
      'author.username': {
        $regex: author,
        $options: 'i'
      },
...
router.get('/:levelId', async (req, res, next) => {
  ...
  let level = await req.app.locals.db.collection('levels').findOne(query)
  if (!level) {
    let levelNotFound = new Error()
    levelNotFound.status = 404
    levelNotFound.errors = 'Level not found'
  ...
  if (!(level.published || req.app.locals.authenticated && req.app.locals.userId == level.author.id)) {
    let userNotAuthorized = new Error()
    userNotAuthorized.status = 403
    userNotAuthorized.errors = 'Levels, that aren\'t published, can only be viewed by authorized users'
...
router.put('/:levelId', checkAuthenticated('Levels can only be updated by authenticated users'),
  checkNoDuplicateVersionIds(), setArtists(), setAuthor(),
  checkAuthorized('Levels can only be updated by authorized users', 'The given user isn\'t authorized to update this level'),
  async (req, res, next) => {
    let query;
    try {
      query = { _id: new ObjectId(req.params.levelId) }
    ...
    const result = await req.app.locals.db.collection('levels').replaceOne(query, req.body)
    if (!result.acknowledged || !result.matchedCount === 1) {
      let unknownServerError = new Error()
...
router.delete('/:levelId', checkAuthenticated('Levels can only be deleted by authenticated users'),
  checkAuthorized('Levels can only be deleted by authorized users', 'The given user isn\'t authorized to delete this level'),
  async (req, res, next) => {
    let query;
    try {
      query = { _id: new ObjectId(req.params.levelId) }
    ...
    const result = await req.app.locals.db.collection('levels').deleteOne(query)
    if (!result.acknowledged || !result.matchedCount === 1) {
      let unknownServerError = new Error()
      unknownServerError.status = 500
      unknownServerError.errors = 'Something went wrong'
...
router.get('/:levelId/:versionId', async (req, res, next) => {
  ...
  if (levels.length === 0) {
    ...
    levelNotFound.message = 'Either no level with levelId ' + req.params.levelId
      + ' found or no version with versionId ' + req.params.versionId + ' of level found'
    return next(levelNotFound)
  }
